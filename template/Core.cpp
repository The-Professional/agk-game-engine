...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_SIZE:
		{
			...
			agk::WindowMoved();
            CSettings::Instance().SetWindowResized( true );
			break;
		}
...
#endif
			unsigned int key = TranslateKey( (UINT)wParam );
			if ( key > 0 )
			{
				agk::KeyDown( key );
			}
...
HWND CreateWin32Window( HINSTANCE hInstance, /*int width, int height,*/ uString &szTitle/*, int x, int y, bool fullscreen */)
{
	...
	// initialise window
	HWND hWnd = CreateWindowW(L"OPENGLWINDOW", L"KittyBall", dwStyle, 0, 0, 
                     ...
                     int       nCmdShow)
{
	...
	try
	{
        hWnd = CreateWin32Window( hInstance, uString( "KittyBall" ) );
    ...
    {
        MessageBoxA( NULL, e.GetErrorMsg().c_str(), e.GetErrorTitle().c_str(), 0 );
        bExitLoop = true;
    }
